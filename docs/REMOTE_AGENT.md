...
Version 0.14 packages `vdl-fleetd` 1.1 for deployments that require a real network listener and distinct subjects. It requires a Keychain server identity, mutual TLS with normal platform trust, an exact certificate SHA-256 for each enabled subject, and viewer/operator/administrator authorization. Its versioned protocol covers worker enrollment, heartbeat, idempotent submission, exclusive claim, monotonic progress, bounded result commit, status query, and queued/running cancellation. Request bodies are capped at 1 MiB and every authorization decision is written to a server-identity-signed SHA-256 audit chain. See `OPERATIONS_HARDENING.md` or run `vdl-fleetd --protocol-json` for the exact endpoint contract.

The companion `vdl-fleetworker` executable implements the client lifecycle with Keychain mTLS, an exact server pin, oldest-job claim, periodic heartbeat/progress, controller cancellation, `vdlctl` execution, and result checksum. The packaged `fleet-worker.example.json` is intentionally non-runnable until its URL, subjects, identity labels, certificate pin, and output root are replaced.
