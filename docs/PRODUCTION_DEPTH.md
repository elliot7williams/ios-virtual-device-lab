...
5. Network and audio faults use typed, bounded scenarios. The backend request is sent only after protocol-v3 authentication, replay protection, and `fault_injection` capability negotiation. The current vphone companion implements timed network-offline with explicit clear/status; latency, loss, proxy, and audio requests remain unsupported and fail closed.
