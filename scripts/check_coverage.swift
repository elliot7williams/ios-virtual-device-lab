...
guard CommandLine.arguments.count >= 2 else {
    fail("usage: check_coverage.swift <llvm-cov.json> [minimum-overall-line-percent] [minimum-expansion-line-percent] [minimum-production-depth-line-percent] [minimum-release-completion-line-percent]")
}
...
let minimumReleaseCompletion = CommandLine.arguments.count > 5 ? Double(CommandLine.arguments[5]) ?? 85 : 85
...
let releaseCompletion = files.first { ($0["filename"] as? String)?.hasSuffix("/ReleaseCompletion.swift") == true }
...
let releaseCompletionPercent = ((releaseCompletion?["summary"] as? [String: Any])?["lines"] as? [String: Any])?["percent"] as? NSNumber
...
guard let releaseCompletionLinePercent = releaseCompletionPercent?.doubleValue, releaseCompletionLinePercent.isFinite else {
    fail("ReleaseCompletion.swift is missing from the coverage report")
}
...
print(String(format: "ReleaseCompletion.swift line coverage %.2f%% (floor %.2f%%)", releaseCompletionLinePercent, minimumReleaseCompletion))
...
guard releaseCompletionLinePercent >= minimumReleaseCompletion else { fail("release-completion line coverage regressed below its floor") }
